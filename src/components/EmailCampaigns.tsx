...
export const EmailCampaigns: React.FC<{ globalLeads: any[], isLoading: boolean }> = ({ globalLeads, isLoading }) => {
  ...
  const [activeFilter, setActiveFilter] = useState<'all' | 'no-web' | 'with-web' | 'contacted' | 'not-contacted'>('all');
  ...
  const filteredLeads = activeFilter === 'no-web'
    ...
    ? leadsWithEmail.filter(l => !!l.emailCampaignContacted)
      ...
      await Promise.all(
        leadsToMark.map((leadId: string) =>
          updateDoc(doc(db, 'leads', leadId), {
            emailCampaignContacted: true,
            emailCampaignContactedAt: new Date().toISOString(),
            emailCampaignTemplate: selectedTemplate?.name || '',
          })
        )
      );
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <button 
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-all ${
              activeFilter === 'not-contacted' ? 'bg-orange-50 border-orange-200 text-orange-600 font-bold' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'
            }`}
          >
            <Mail className="w-4 h-4" />
            Sin Contactar ({leadsWithEmail.filter(l => !l.emailCampaignContacted).length})
          </button>
          <button 
            onClick={() => setActiveFilter('contacted')}
            className={`flex items-center gap-2 px-4 py-2 rounded-lg border transition-all ${
              activeFilter === 'contacted' ? 'bg-blue-50 border-blue-200 text-blue-600 font-bold' : 'bg-white border-gray-200 text-gray-600 hover:border-gray-300'
            }`}
          >
            <CheckCircle2 className="w-4 h-4" />
            Contactados ({leadsWithEmail.filter(l => !!l.emailCampaignContacted).length})
          </button>
        </div>
      </div>
                      ...
                      <div className="flex flex-col gap-1">
                        ...
                        {lead.emailCampaignContacted && (
                          <span className="text-[10px] px-2 py-0.5 bg-blue-100 text-blue-700 rounded-full font-bold flex items-center gap-1">
                            <Mail className="w-2.5 h-2.5" /> CONTACTADO
                          </span>
                        )}
                      </div>
                    </td>
